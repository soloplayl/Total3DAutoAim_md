...
```mermaid
 ...
 ```
### 了解模型搭建原理先验知识需要知道罗德里格斯公式(旋转向量转旋转矩阵)，旋转向量和旋转矩阵的定义和关系，平移向量的定义(可以参考https://zhuanlan.zhihu.com/p/141597984，https://zhuanlan.zhihu.com/p/677674288和https://zhuanlan.zhihu.com/p/451579313)
...
# 缺点和未完善点
- 仅在模型时序预测上，我想谈一下我最近尝试了一下模型以及发现了模型的某些缺点。
- 首先虽然大小符都可以预测，但是对于大符的时序预测，仅使用坐标信息如果时间窗口较小的话无法达到预测目标。具体表现为模型会当做一个时不变预测系统，且由于观测窗口较小，相当于会出现kalman那种滞后预测(即直接线性预测)。
- 具体我的改善做法是有两种。
- 第一种: 由于transformer本身就是一种长时间序列的模型，因此将输入改为70个时间序列点，前60个时间点，用核窗口为3，步距为3的average pooling的方式压缩为了20个时间点加上后十个时间数据拼接成30个时间点数据。
这样做是为避免加剧模型计算量，且维持模型时间序列的时间间隔，可以使得模型在时序预测上有更好的效果。
- 第二种: 当然你也可以直接进行角速度预测这样就可以避免这个问题(前提是你的角速度函数不能失真)，虽然这样就会偏离了我最初的想法。
- ![img.png](img.png)![img_1.png](img_1.png)
- 其次是模型的天然缺陷。由于数据集是对于连续空间的离散采样，本身的观测时间就是一个变量。因此实际上我在模型推理输入时是线性插值的方法固定时间观测和统计时间分布的两种做法来解决的。结论是线性插值的方法固定时间观测的效果更好。
- 线性插值固定时间观测: 比如你的代码运行一次需要10ms±3ms，那么可以选取10ms作为你的观测时间，那么你每次观测时便将你的数据线性插值到10ms的时间点上，这样便可以避免模型对于时间的敏感性，好处是可以用生成数据去训练，不用收集数据。
- 统计时间分布: 你可以统计你的代码运行时间的分布，但是由于即使你统计的时间分布，但是你不知道具体时间是什么分布的特征，你利用模型数据集生成器生成数据作为训练集效果和真实的有所差距，具体表示为预测相位差。因此你需要收集真实的数据集来训练模型。但这无疑增加了数据集的收集成本。
- 未完善点或者猜想:
- 正如我刚才所说的数据集是对于连续空间的离散采样，本身的观测时间就是一个变量。RM大符预测很多使用的是最小二乘法，最小二乘法使用的前提是已知模型，进行参数估计得到的是一个连续函数。那么是否可以将离散模型改为连续的Neural ode来进行预测呢，这样便可以避免模型对于时间的敏感性，而且可以预测任何时间点的数据，方便了模型的推理预测。
## *打了两年的感想*
