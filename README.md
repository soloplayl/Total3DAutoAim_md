...
- 模型的输入是[batch=1,seq_len=输入数据的时间步，这个可以自定义,7=[[t-t[0],x-x[0],y-[0],z-[0],rot]]]。可以看出模型输入数据实际上是都减去的初始值也就是减去比如收集10个数据的初值(除旋转矩阵不减)，因此我们数据的坐标系此时是第一个装甲作为初始点的坐标系，也就是我下面图中写到目标装甲板坐标系，所以需要保存第一个数据的值即为初值用来还原为自身车坐标系数据。
