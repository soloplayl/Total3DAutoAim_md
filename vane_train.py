...
def create_dataset(data, input_size=10, output_size=10, offset=40):
    ...
    x_diff = (np.cos(data['theta']) * np.sin(data['phi']))[:, :,np.newaxis] # (4000, 200, 1)
    y_diff = (np.cos(data['theta']) * np.cos(data['phi']))[:, :, np.newaxis] # (4000, 200, 1)
    z_diff = (np.sin(data['theta']))[:,:, np.newaxis] # (4000, 200, 1)
...
# ==================== 5. 配置参数 ====================
config = {
    'data_path': 'model/datasets/vane_dataset5000.npz',
    ...
    'epochs': 100,
    'lr': 1e-4,
    'model_type': 'vane_transformer', # vane_transformer
    ...
    'd_model': 256,
    'n_heads': 8,
    'd_ff': 256, # 前馈网络维度这个ffn层设为512的的话建议训练所有状态学习，单学习能量机关256即可，如果预测自瞄建议512
    'num_layers': 3,
    'eta_min' : 1e-5 # 余旋退火参数
}
# --- 冻结配置字典（0表示冻结，1表示可训练）---
